...
class User32 {
  ...
  public static Preload(methods?: (keyof typeof User32.Symbols)[]): void {
    ...
    const symbols = Object.fromEntries(
      methods
        .filter((method) => Object.getOwnPropertyDescriptor(User32, method)?.configurable !== false)
        .map((method) => [method, User32.Symbols[method]])
    );
  ...
  // Install lazy getters for all symbols: first access binds and memoizes.
  static {
    for (const method of Object.keys(User32.Symbols) as (keyof typeof User32.Symbols)[]) {
      Object.defineProperty(User32, method, {
        configurable: true,
        get() {
          return User32.Load(method);
        },
      });
    }
  }
